#!/usr/bin/env python3
"""Attach one App Review screenshot per Next Cue Pro product.

Each product's screenshot has to show that product's own billed amount, so the
three renders from `PaywallSnapshotUITests.testPaywallSnapshots` go to their own
products rather than one image going to all of them.

    python3 scripts/asc-upload-review-screenshots.py --dir app-store/iap-review
...
def main() -> None:
    ...
    parser.add_argument("--dir", default="app-store/iap-review", help="folder holding the renders")
